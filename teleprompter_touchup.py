...
class TeleprompterHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif self.path == "/api/touchup-status":
            # Check if Touch-Up app is running
            if platform.system() == "Darwin":
                try:
                    running = False
                    detail = ""
                    # Method 1: pgrep for common Touch-Up process names
                    for pattern in ["Touch-Up", "Touch Up", "TouchUp"]:
                        r = subprocess.run(
                            ["pgrep", "-f", pattern],
                        ...
                        if r.returncode == 0:
                            running = True
                            break
                    # Method 2: AppleScript process list
                    if not running:
                        r2 = subprocess.run(
                            ["osascript", "-e",
                             'tell application "System Events" to return '
                             'name of every process whose name contains "Touch"'],
                            capture_output=True, text=True, timeout=3
                        )
                        detail = r2.stdout.strip()
                        if "Touch" in detail and "Up" in detail:
                            running = True
                    self._json_response({
                        "running": running,
                        "detail": detail
                ...
                self._json_response({"running": False, "error": "macOS only"})
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .teleprompter {
    ...
    font-size: 2.8rem;
    position: relative;
    /* No touch-action:none — Touch-Up driver handles gesture recognition
       and injects native scroll/click events that the browser handles. */
  }
  .teleprompter .inner {
    max-width: 1800px;
    margin: 0 auto;
...
<div id="touchupBanner" style="display:none;background:#b45309;color:#fff;text-align:center;padding:6px 12px;font-size:0.85rem;cursor:pointer;z-index:999;" onclick="this.style.display='none'">
  <span id="touchupMsg">Checking for Touch-Up driver...</span>
  <span style="margin-left:12px;opacity:0.7;">[click to dismiss]</span>
</div>
...
// ── Touch-Up driver detection ──
(function() {
  var banner = document.getElementById('touchupBanner');
  var msg = document.getElementById('touchupMsg');
  if (!banner || !msg) return;

  fetch('/api/touchup-status', { method: 'POST' })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      banner.style.display = 'block';
      if (data.running) {
        banner.style.background = '#166534';
        msg.textContent = 'Touch-Up driver detected — touchscreen gestures enabled (flick to scroll, tap to click)';
      } else {
        banner.style.background = '#b45309';
        msg.innerHTML = 'Touch-Up driver not detected — touchscreen may not scroll/click properly. '
          + '<a href="https://github.com/shueber/Touch-Up/releases" target="_blank" '
          + 'style="color:#fff;text-decoration:underline;">Download Touch-Up</a>';
      }
      // Auto-dismiss after 15 seconds if Touch-Up is running
      if (data.running) {
...
// ── Touch-Up touchscreen support ──
// With the Touch-Up driver (github.com/shueber/Touch-Up) running,
// flick gestures generate native scroll events and taps generate clicks.
// The browser's overflow-y:auto handles scrolling natively — no custom
// drag handler needed for touch input.
//
// Mouse-drag fallback: still useful when using a regular mouse without
// Touch-Up, or as a backup if Touch-Up isn't running.
(function() {
  var tp = document.getElementById('teleprompter');
  var dragging = false;
  var lastY = 0;
  ...
  // Only handle mouse-button drags (not touch — Touch-Up handles that)
...
def main():
    ...
    print(f"\nTeleprompter (Touch-Up edition) running at http://localhost:{port}")
    print(f"\n  Phone remote:  http://{local_ip}:{port}/remote")
    print(f"  QR code page:  http://localhost:{port}/qr")

    # Check for Touch-Up driver
    if platform.system() == "Darwin":
        try:
            r = subprocess.run(["pgrep", "-f", "Touch"], capture_output=True, timeout=3)
            if r.returncode == 0:
                print("\n  Touch-Up driver: DETECTED — touchscreen gestures should work")
            else:
                print("\n  Touch-Up driver: NOT RUNNING")
                print("  For touchscreen support, download from:")
                print("  https://github.com/shueber/Touch-Up/releases")
                print("  Launch Touch-Up.app before using the touchscreen.")
        except Exception:
            pass
